...
func TestGatewayServiceRecordUsage_PrefersBillingRequestIDWhenWorkerContextIsDetached(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	svc := newGatewayRecordUsageServiceWithBillingRepoForTest(usageRepo, billingRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{})
	...
	err := svc.RecordUsage(context.Background(), &RecordUsageInput{
		...
		User:             &User{ID: 601},
		Account:          &Account{ID: 701},
		BillingRequestID: "client:stable-request-123",
	})
	require.NoError(t, err)
	require.NotNil(t, billingRepo.lastCmd)
	require.Equal(t, "client:stable-request-123", billingRepo.lastCmd.RequestID)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "client:stable-request-123", usageRepo.lastLog.RequestID)
}

func TestGatewayServiceRecordUsage_NormalizesLongBillingRequestID(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	svc := newGatewayRecordUsageServiceWithBillingRepoForTest(usageRepo, billingRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{})

	longRequestID := "local:" + strings.Repeat("codex-long-billing-request-id-", 4)
	err := svc.RecordUsage(context.Background(), &RecordUsageInput{
		Result: &ForwardResult{
			RequestID: "upstream-ignored",
			Usage: ClaudeUsage{
				InputTokens:  10,
				OutputTokens: 6,
			},
	...
	require.True(t, strings.HasPrefix(billingRepo.lastCmd.RequestID, "local-h:"))
