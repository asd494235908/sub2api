...
const requireFromVueI18n = createRequire(createRequire(import.meta.url).resolve('vue-i18n/package.json'))
const { baseCompile } = requireFromVueI18n('@intlify/message-compiler') as {
  baseCompile: (source: string, options: { onError: (error: { message: string }) => void }) => unknown
}

function collectCompileErrors(node: unknown, path: string, out: string[]): void {
  if (typeof node === 'string') {
    baseCompile(node, {
      onError: (error) => out.push(`${path}: ${error.message}`)
    })
    return
  }
