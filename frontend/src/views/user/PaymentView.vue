            ...
            <div v-if="validAmount > 0" class="card p-6">
              <div class="space-y-2 text-sm">
                ...
                <div v-if="balanceRechargeMultiplier !== 1" class="flex justify-between" :class="{ 'border-t border-gray-200 pt-2 dark:border-dark-600': feeRate <= 0 }">
                  ...
                  <span class="text-gray-900 dark:text-white">{{ t('payment.platformQuotaAmount', { amount: creditedAmount.toFixed(2) }) }}</span>
                </div>
                  ...
                  <div class="card p-5">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <div class="text-left sm:text-right">
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('payment.firstRecharge.description') }}</p>
                      </div>
                    </div>
                  </div>
                  <div v-if="enabledMethods.length === 0" class="card py-16 text-center">
                      ...
                      <div class="grid grid-cols-3 gap-2">
                        ...
                        >
                          ...
                          <span class="mt-1 block text-xs font-semibold leading-4 text-emerald-600 dark:text-emerald-400">
                            {{ t('payment.firstRecharge.tierBonus', { amount: tier.bonus_amount.toFixed(2) }) }}
                          </span>
                        </button>
                      </div>
                    ...
                    <div v-if="validAmount > 0" class="card p-6">
                      <div class="space-y-2 text-sm">
                        ...
                        <div v-if="firstRechargeBonus > 0" class="flex justify-between border-t border-gray-200 pt-2 dark:border-dark-600">
                          <span class="text-gray-500 dark:text-gray-400">{{ t('payment.firstRecharge.title') }}</span>
                          <span class="font-semibold text-emerald-600 dark:text-emerald-400">+{{ t('payment.platformQuotaAmount', { amount: firstRechargeBonus.toFixed(2) }) }}</span>
                        </div>
                        <p v-if="balanceRechargeMultiplier !== 1" class="border-t border-gray-200 pt-2 text-xs text-gray-500 dark:border-dark-600 dark:text-gray-400">
                          {{ t('payment.rechargeRatePreview', { usd: balanceRechargeMultiplier.toFixed(2) }) }}
                        </p>
...
const checkout = ref<CheckoutInfoResponse>({
  ...
  plans: [], balance_disabled: false, balance_recharge_multiplier: 1, subscription_usd_to_cny_rate: 0, recharge_fee_rate: 0, help_text: '', help_image_url: '', stripe_publishable_key: '', test_recharge_enabled: false,
})
