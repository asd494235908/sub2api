      ...
      >
        <a href="#top" class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-lg font-black tracking-tight">格品API</p>
          </div>
        </a>
        ...
        >
          ...
          <div class="mt-6 rounded-[1.25rem] border border-sky-200 bg-sky-50 p-5 dark:border-cyan-400/20 dark:bg-cyan-500/10">
            <p class="text-sm leading-7 text-sky-900 dark:text-cyan-100">
              📌 说明：格品API 为标准 RESTful API 服务，只要设备能发起 HTTPS 请求即可使用，不限操作系统。
            </p>
          </div>
        </article>
      </section>
          ...
          <div class="space-y-5">
            <article class="rounded-[1.5rem] border border-slate-200/80 bg-slate-950 shadow-sm dark:border-white/10">
              ...
              <div class="space-y-3 px-6 py-5 font-mono text-sm leading-7 text-slate-200">
                ...
                <div class="pl-4"><span class="text-slate-400">messages=</span>[{ <span class="text-slate-400">"role"</span>: <span class="text-emerald-300">"user"</span>, <span class="text-slate-400">"content"</span>: <span class="text-emerald-300">"你好，格品API！"</span> }]</div>
                <div>)</div>
...
const navItems = [
  ...
  { href: docUrl, label: 'API文档', external: true },
...
const heroAdvantages = [
  {
    icon: '🔒',
    title: '公司正规运营',
    description: '稳定的售后服务、签合同、SLA 99.9% 可用性保障'
  },
  {
    icon: '⚡',
    title: '私有代理加速',
...
const benefitRows = [
  ...
  { title: '合规安心', description: '支持合同、稳定的售后服务与企业结算。' }
]
...
const ecosystemRows = [
  { tool: 'Codex', integration: '填入 格品API Base URL 与 API Key 即可接入', tutorial: '已支持' },
  { tool: 'VS Code', integration: '通过 OpenAI 兼容配置接入 格品API', tutorial: '已支持' },
  { tool: 'Chatbox', integration: '选择 OpenAI 兼容方式并填写端点与密钥', tutorial: '已支持' },
  { tool: 'OpenCode', integration: '自定义 API Provider 指向 格品API', tutorial: '已支持' }
]
...
const docsSteps = [
  {
    ...
    description: '登录 格品API 控制台 → 左侧菜单「API 密钥」→ 点击「创建新密钥」\n复制保存形如 sk-gepinapi-xxxxxxxxxxxxxxxx 的密钥串'
  },
  {
    title: 'Step 2：确定接入端点',
    description: '格品API 网关地址：https://token.gepinapi.com/v1\n该地址与 OpenAI 官方格式 100% 兼容'
  },
  {
