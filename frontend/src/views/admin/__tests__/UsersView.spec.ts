...
vi.mock('vue-i18n', async () => {
  ...
  return {
    ...actual,
    useI18n: () => ({
      t: (key: string) => key === 'admin.users.phone' ? 'Phone' : key
    })
  }
})
...
describe('admin UsersView', () => {
  ...
  it('includes phone_number as a visible admin users column', async () => {
    const wrapper = mountPhoneUsersView()
    await flushPromises()
  ...
  it('allows sorting by phone_number and passes phone search terms through', async () => {
    ...
    expect(listUsers).toHaveBeenLastCalledWith(
      1,
      20,
      expect.objectContaining({ sort_by: 'phone_number', sort_order: 'asc' }),
      expect.any(Object)
    )
    ...
    expect(listUsers).toHaveBeenLastCalledWith(
      1,
      20,
      expect.objectContaining({ search: '138001380' }),
      expect.any(Object)
    )
  })

  it('uses the localized phone label for the phone_number column', async () => {
    const wrapper = mountPhoneUsersView()
    await flushPromises()

    expect(wrapper.get('[data-test="column-labels"]').text()).toContain('Phone')
  })
})
