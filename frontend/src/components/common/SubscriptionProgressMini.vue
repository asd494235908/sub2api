          ...
          >
            ...
            <div class="space-y-1.5">
              ...
              <template v-else>
                ...
                <div v-if="subscription.total_limit_usd && subscription.total_limit_usd > 0" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-gray-500">{{
...
function getMaxUsagePercentage(sub: UserSubscription): number {
  ...
  if (sub.total_limit_usd && sub.total_limit_usd > 0) {
    percentages.push(((sub.total_usage_usd || 0) / sub.total_limit_usd) * 100)
  }
...
function isUnlimited(sub: UserSubscription): boolean {
  return (
    ...
    !(sub.total_limit_usd && sub.total_limit_usd > 0)
  )
}
