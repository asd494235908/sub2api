...
router.beforeEach(async (to, _from, next) => {
  ...
  // Set page title after public settings and auth state are ready.
  ...
  // 无 __APP_CONFIG__ 注入）。此时 cachedPublicSettings 为空会把 payment/lucky wheel/risk_control
  // 误判为“未启用”而错误拦截，故这里先确保设置加载完成。
  if ((to.meta.requiresPayment || to.meta.requiresLuckyWheel || to.meta.requiresRiskControl) && !appStore.publicSettingsLoaded) {
    try {
  ...
  if (
    to.meta.requiresLuckyWheel &&
    appStore.publicSettingsLoaded &&
    appStore.cachedPublicSettings?.lucky_wheel_enabled === false
  ) {
    next(authStore.isAdmin ? '/admin/dashboard' : '/dashboard')
    return
  }
