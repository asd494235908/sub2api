...
describe('feature route guard', () => {
  ...
  it('marks the user lucky wheel route with the feature requirement', () => {
    expect(routerHarness.routes.find(route => route.path === '/lucky-wheel')?.meta).toMatchObject({
      requiresPayment: true,
      requiresLuckyWheel: true,
    })
    expect(routerHarness.routes.find(route => route.path === '/admin/orders/lucky-wheel')?.meta).not.toHaveProperty('requiresLuckyWheel')
  })
  ...
  it.each([
    ...
    [
      'lucky wheel',
      { requiresPayment: true, requiresLuckyWheel: true },
      { payment_enabled: true, lucky_wheel_enabled: false },
      '/dashboard',
    ],
    [
      'risk control',
  ...
  it('allows lucky wheel navigation when payment and the feature are enabled', async () => {
    appStore.cachedPublicSettings = { payment_enabled: true, lucky_wheel_enabled: true }
    appStore.publicSettingsLoaded = true
